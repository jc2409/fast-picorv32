...
def ask_binary_param(param):
    while True:
        ans = input(f"Parameter {param}: 0 (fixed 0), 1 (fixed 1), v (variable): ").strip().lower()
        if ans in ('0', '1', 'v'):
            return ans
        print("Invalid input. Please enter 0, 1, or v.")
...
def generate_configs(binary_choices, mem_words_values, cache_lines_vals, cache_words_vals, cache_modules):
    ...
    for param in BINARY_PARAMS:
        choice = binary_choices[param]
        vals = [0] if choice == '0' else [1] if choice == '1' else [0,1]
        binary_lists.append(vals)
        binary_names.append(param)
    combos = list(product(*binary_lists))
    rows = []
    for combo in combos:
        row = {}
        for i, p in enumerate(binary_names):
            row[p.lower()] = combo[i]
        name_parts = [p.lower() for i, p in enumerate(binary_names) if combo[i]]
        name = "_".join(name_parts) if name_parts else "baseline"
        suffix = ''.join(str(b) for b in combo)
        row['config_name'] = f"{name}_{suffix}"
        rows.append(row)
    ...
    if mem_words_values is not None:
        ...
        for r in rows:
            for mw in mem_words_values:
                nr = r.copy()
                nr['mem_words'] = mw
                # Update config_name to include mem_words
    ...
    # Write CSV
    fieldnames = ['config_name'] + [p.lower() for p in BINARY_PARAMS]
    if mem_words_values is not None:
